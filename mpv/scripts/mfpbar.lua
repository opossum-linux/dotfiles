...
local function pbar_update(next_state)
	...
	if (dpy_w == 0 or dpy_h == 0 or
	    state.pbar == next_state or state.pbar == PBAR_UNINIT or state.inhibit_count > 0 or state.player_idle)
	then
		return
	end
...
local function update_idle_active(kind, idle)
	zassert(kind == "idle-active")
	if (idle) then
		pbar_update(PBAR_HIDDEN)
		state.player_idle = true
	else
		state.player_idle = false
	end
end
...
local function init()
	...
	mp.observe_property("idle-active", "native", update_idle_active)
end
